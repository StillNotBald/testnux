...
TrunkNuX is the tool that produces audit-defensible evidence chains. As of v0.2.2 it didn't dogfood itself. This pass closes that gap. The project now uses its own discipline on its own surface, and every artifact lives in the public repo.
...
- **`requirements/TRACEABILITY.md`** — generated by `trunknux rtm`. 58/58 R-IDs have test evidence (100% test coverage); 3 have inline code annotations (`// R-XX`); 0 sprint-log entries (sprint-log discipline not yet adopted on this project — flagged).
- **`testing-log/2026-04-27_trunknux-cli/`** — 60 TCs (TRUNK-01 through TRUNK-60) mapped to vitest test surfaces. All 60 PASS. Includes generated XLSX (13KB) and self-contained HTML execution report (120KB).
- **`sca/trunknux-cli.md`** — 295-line Security Control Assessment. References 36 R-IDs across the discipline, audit-chain, and quality groups. Identifies **7 honest gaps** in the current implementation including missing `// R-XX` source annotations, no pre-publish secrets grep, no `UAT_SECRET` rotation mechanism, `--config` flag broken on Windows, and a `<style>` tag in the DOMPurify allowlist (CSS exfiltration vector at PDF render time).
- **`testing-log/2026-04-27_trunknux-cli/uat-log.jsonl`** — 60 HMAC-chained attestation entries (one per TC) signed by Chu Ling as Founder for the v0.2.2 release. Hash chain verified valid via `verifyChain()`.
- **`requirements/validations/trunknux-cli/v1.0.oscal.json`** — NIST OSCAL 1.1.2 assessment-results document, ready for IBM Compliance Trestle import or Vanta / Drata / Secureframe / RegScale ingestion.

PDF render not produced this pass: `puppeteer-core` is not a dependency. To render `sca/trunknux-cli.pdf` and the signoff PDF locally, run `npm install puppeteer-core` and re-run `trunknux sca pdf trunknux-cli` and `trunknux sign pdf trunknux-cli`. The markdown SCA and JSONL ledger are the canonical source of truth.
